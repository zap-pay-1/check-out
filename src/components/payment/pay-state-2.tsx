...
import { useModal, useWallets} from '@particle-network/connectkit'
...
const [klasterAddress, setklasterAddress] = useState(null)
const [klasterBalances, setklasterBalances] = useState(null)
// WALLET CLIENT
...
useEffect(() => {
  const getBalance = async ()  =>  {
    try {
      ...
      const uBalance = await mcClient.getUnifiedErc20Balance({
       ...
       setklasterBalances(readableBalnce)
       console.log("balances",  readableBalnce)
   console.log("full balances",  uBalance)
      
    }} catch (error) {
      console.log("error when getting balance", error)
    }
  ...
  const {data : countries, isError : isCountriesError, isSuccess : isCountriesSuccess, isLoading : isCountriesLOading, error : countriesErro}  = useQuery({
   ...
   const  AMOUNT_2 = "0.001"   //data?.session?.amount || data?.reciever?.amount
        ...
        if( data?.reciever?.collectEmail ||  
            ...
            data?.reciever?.collectAddress ){
                return(
                   ...
                   <button onClick={() => disconnect()} className='py-3 px-6 bg-yellow-500'>disconnect</button>
                   <p>{address}</p>
                    <div> 
